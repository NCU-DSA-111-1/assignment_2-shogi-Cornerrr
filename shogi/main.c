...
{   
    ...
    bool start = 1;
    char leave;
    char arr1[]={"[紅]是否離開,離開請按l,不離開請按n,儲存請按s:"};
    record =arr1;
    timemain();
    scanf(" %c",&leave);
    if(leave == 'l'){
    ...
    if(restart){
        //printf("[紅]違反遊戲規則請重新輸入\n");
        char arr30[]={"[紅]違反遊戲規則請重新輸入"};
        record2 =arr30;
        restart = 0;
    }
    if(list->left != NULL ){    //回到上一手
        //printf("是否回到上一手[y/n]: ");
        char arr2[]={"[紅]是否回到上一手[y/n]: "};
        record =arr2;
        timemain();
        scanf(" %c",&yn);
        if(yn == 'y'){
...
int blueMove(FILE *fptr)
{
    ...
    if(restart){
        //printf("[藍]違反遊戲規則請重新輸入\n");
        char arr31[]={"[藍]違反遊戲規則請重新輸入"};
        record3 =arr31;
        restart = 0;
    }
    if(list->left != NULL ){    //回到上一手
        char arr8[]={"[藍]是否回到上一手[y/n]: "};
        record =arr8;
        timemain();
        //printf("是否回到上一手[y/n]: ");
        scanf(" %c",&yn);
    ...
    for(int i=1;i<3;i++){
        for(int j=1;j<11;j++){
            if(tophandchek(i,j)==3){
                ...
                char arr15[]={"[紅]是否選擇打入棋子[y/n]: "};
                record =arr15;
                timemain();
                scanf(" %c",&yn);
                if(yn=='y'){
                    //printf("請輸入tophand想打入的棋子座標(行): ");
                    char arr16[]={"請輸入tophand想打入的棋子座標(行): "};
                    record =arr16;
                    timemain();
                    scanf("%d",&xi);
                    //printf("請輸入tophand想打入的棋子座標(列): ");
                    char arr17[]={"請輸入tophand想打入的棋子座標(列): "};
                    record =arr17;
                    timemain();
                    scanf("%d",&yi);
                    //printf("請輸入tophand想放到的棋盤座標(行): ");
                    char arr18[]={"請輸入tophand想放到的棋盤座標(行): "};
                    record =arr18;
                    timemain();
                    scanf("%d",&xj);
                    //printf("請輸入tophand想放到的棋盤座標(列): ");
                    char arr19[]={"請輸入tophand想放到的棋盤座標(列): "};
                    record =arr19;
                    timemain();
                    scanf("%d",&yj);
    ...
    for(int i=0;i<2;i++){
        for(int j=1;j<11;j++){
            if(bothandchek(i,j) == 4){
                ...
                char arr20[]={"[藍]是否選擇打入棋子[y/n]: "};
                record =arr20;
                timemain();
                scanf(" %c",&yn);
                if(yn=='y'){
                    //printf("請輸入bothand想打入的棋子座標(行): ");
                    char arr21[]={"請輸入bothand想打入的棋子座標(行): "};
                    record =arr21;
                    timemain();
                    scanf("%d",&xi);
                    //printf("請輸入bothand想打入的棋子座標(列): ");
                    char arr22[]={"請輸入bothand想打入的棋子座標(列): "};
                    record =arr22;
                    timemain();
                    scanf("%d",&yi);
                    //printf("請輸入bothand想放到的棋盤座標(行): ");
                    char arr23[]={"請輸入bothand想放到的棋盤座標(行): "};
                    record =arr23;
                    timemain();
                    scanf("%d",&xj);
                    //printf("請輸入bothand想放到的棋盤座標(列): ");
                    char arr24[]={"請輸入bothand想放到的棋盤座標(列): "};
                    record =arr24;
                    timemain();
                    scanf("%d",&yj);
                    if(xi>2 || yi>10 || xj>9 || yj>9){
